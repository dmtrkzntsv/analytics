...
# exiting nonzero, a corrupt download — must leave the previous replica
# byte-for-byte intact. The last case runs in loop mode, where the
# `restore_once || …` caller disables set -e inside the function; that path
# once installed an empty database over a good replica.
...
case "${LITESTREAM_STUB_MODE:-ok}" in
  ok)      sqlite3 "$out" 'CREATE TABLE t(x); INSERT INTO t VALUES (1);' ;;
