...
func run(args []string, stdout io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stdout, "usage: analytics <serve|dashboards|migrate|keygen|project|key|config|version> [flags]")
		return 2
	}
	cmd, ok := commands[args[0]]
	if !ok {
		fmt.Fprintf(stdout, "unknown command %q\nusage: analytics <serve|dashboards|migrate|keygen|project|key|config|version> [flags]\n", args[0])
		return 2
	}
